...
EXPECTED_V2_PROMPT = """You are a query planning model.
...
def test_query_planning_v2_prompt_is_exact():
    prompt_path = Path("prompts/query_planning_v2_exact.txt")
    actual = prompt_path.read_text(encoding="utf-8")
    if actual.endswith("\n"):
        actual = actual[:-1]
    assert actual == EXPECTED_V2_PROMPT
